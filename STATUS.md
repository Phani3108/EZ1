...
|---|---|---|
| Architectural cleanup (Phase 20) | ✅ ~complete | `shared/eduzim_shared/routes.py` centralizes `_meta`/`_err`/`_ok`/`make_audit_helper`/`CROSS_SCHOOL_SENTINEL` (32 routes files migrated; −380 net LoC). `shared/eduzim_shared/permissions.py` ships `Perm` enum + drift-guard test against the gateway RBAC table (kills typo-denial security risk). Per-service `tests/conftest.py` (additive — pytest fixture-resolution preserves existing per-file overrides; Phase 21 codemod deletes the duplicates). `.env.example` regenerated from `grep os.environ` sweep; every env var the codebase reads is documented + grouped REQUIRED/optional. **887 tests passing** (academics 495 + comm 145 + finance 90 + identity 39 + shared 118 incl. 7 new permission tests). |
...
| Curriculum & Question Bank | ✅ ~97% (Phase 18, 2026-05-27) | Phase 16a-i + Phase 17a-e closed. **Models**: Subject extended with grade_levels + national_subject_id + adopted_national_version; new Unit, Topic, NationalSubject (with `version`), NationalUnit, NationalTopic, Question, QuestionOption, QuestionDraft, HomeworkTemplate, LessonPlanTemplate. Alembic migrations `2026_05_27_021_curriculum.py` (Phase 16) + `2026_05_27_022_curriculum_versioning.py` (Phase 17d). **Endpoints**: school-local CRUD (`/curriculum/{subjects,units,topics}`, `/curriculum/tree`, `/curriculum/adopt-subject`); Ministry-side (`/ministry/national-curriculum/*` gated `school:create`; `/republish` bumps version in Phase 17d); bulk CSV (`/bulk/curriculum` + `/ministry/bulk/national-curriculum`); cross-index (`/curriculum/topics/{id}/resources` + `/curriculum/coverage`); generic tag (`/curriculum/tag`); question bank (`/questions`, `/question-drafts` + approve/reject); assessment composition + auto-grading (`/assessments/{id}/compose` + `/assessments/{id}/auto-grade` covering MCQ/TF/SA); homework + lesson-plan templates (`/homework-templates` + `/lesson-plan-templates`) with HoD-publish + instantiate-to-class + sync-from-source; **Phase 17d**: `/curriculum/upgrade-subject` (idempotent, preserves local custom topics + locally-deleted national rows) + `is_stale` indicator on `GET /curriculum/subjects`. **Attachment polish (Phase 16e + 17a/b)**: MIME allowlist extended with Office (.xlsx/.docx/.pptx/.xls) + text/csv; size cap 10MB → 25MB; 8 new owner_kinds; **Phase 17a**: `POST /comm/attachments/clone` + `/clone-bulk` with `attachment_client.py` cross-service wrapper (best-effort; `EDUZIM_DISABLE_CROSS_SERVICE_HTTP=1` for tests); template instantiation now clones the full attachment bundle; **Phase 17b**: Pillow-based 256×256 JPEG thumbnails with EXIF-orientation handling, `Attachment.thumb_uri` column, `GET /comm/attachments/{id}/thumbnail` with 1-hour private cache. **UI**: teacher-web `/curriculum`, `/curriculum/[topicId]`, `/question-bank`, `/question-bank/drafts`; admin-web `/curriculum`, `/curriculum/coverage`, `/curriculum/adopt`, `/question-bank/review`; admin-web Ministry `/ministry/curriculum` with publish + new-subject modal + **Phase 17c** `/ministry/curriculum/import` drag-drop CSV with dry-run preview. **Sample data**: `scripts/import_zimsec.py` ships 3 placeholder ZIMSEC subjects for dev/test with a "replace before production" notice. **Tests**: +70 backend tests since Phase 15 (751 total: 477 academics + 145 communications + 90 finance + 39 identity). **Audit invariants** (ADR 018, ADR 022, ADR 023): no question text, no answers, no template titles, no homework bodies, no filenames, no syllabus content in `target` / `details`; new Phase 17 events (`attachment.cloned`, `attachment.cloned_bulk`, `national_curriculum.subject.republished`, `curriculum.subject.upgraded`) carry IDs + counts + version deltas only. **Phase 18 additions**: (18a) `GET /curriculum/upgrade-subject/preview` returns the diff of the upcoming upgrade — units_to_add/update, topics_to_add/update with field-level `changed_fields`, plus counts of local-custom + nationally-orphaned rows that will be preserved. Admin-web `/curriculum/[subjectId]/upgrade-preview` page renders the diff with confirm CTA; main `/curriculum` page shows version badges + "Review upgrade" alert when `is_stale === true`. (18b) New global `national_homework_templates` + `national_lesson_plan_templates` (Alembic `2026_05_27_023`) with Ministry CRUD + publish/archive (gated `school:create`) and school-side browse + adopt (gated `school:manage`). Subject + topic codes resolved per-school at adopt-time; idempotent by `(school_id, source_national_template_id)`; unresolved topic codes returned for HoD patching. Per-school `homework_templates` + `lesson_plan_templates` extended with `source_national_template_id` back-ref. v1 is text-only (no cross-tenant attachment cloning — deferred). (18c) Admin-web `/ministry/templates` catalog with publish/archive + `/ministry/templates/new` form; `/templates/national` HoD-facing browse + adopt page. **Open follow-ups** (Phase 19+): OCR (Tesseract), cross-tenant attachment sharing, versioned national templates, bulk CSV national templates, essay auto-grading, JSONB GIN-index swap on Postgres, S3 storage backend. |
| Onboarding (new-school day-1) | ✅ ~95% (Phase 15, 2026-05-27) | Phase 15a–15e closed. **Backend** (15a): Invitation model + endpoints (`services/identity`), StudentDraft + ParentDraft queue with admin approval, bulk teacher import + parent invite auto-queue, bulk fee structure import in finance, InviteDispatcher (SMS→WhatsApp→Email→Manual) with InviteOutbox in communications, 7-check onboarding-readiness API + School.is_live + POST /onboarding/go-live audit-logged. **Roles** (15a): new `Provisioner` (additive to Ministry, school:create+invite:write) and `EduZimOps` super-role (ADR 021); `student:draft` added to Teacher. **Gateway** (15a): RBAC + SERVICE_ROUTES wired with public preview/accept paths. **UI** (15b): `<Stepper>`, `<ChecklistItem>`, `<ReadinessBar>`, `<InvitationStatusPill>` primitives in @eduzim/ui. **admin-web** (15b): `(admin)/setup/**` wizard — 9 pages (landing, school, classes-subjects, teachers, students, parents, fees, drafts, go-live). **parent-web + teacher-web** (15c): `/invite/[token]` magic-link landing + `/invite/code` 6-digit fallback. **Ministry** (15d): `(ministry)/ministry/onboarding/page.tsx` with school grouping (live/setup/new) + "+ New School" modal for Provisioner / EduZimOps. **Tests**: +50 backend tests (681 total: 427 academics + 39 identity + 90 finance + 125 communications). **Audit invariants** (ADR 018): user.invited / user.activated / student.draft.* / invitation.dispatched / school.went_live all PII-free in target+details; rejection_reason is the only admin-supplied free text. **Open follow-ups**: server-side Excel parsing, OCR PDF parsing, channel-config UI, ZW E.164 normaliser. |
