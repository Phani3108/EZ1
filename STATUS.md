...
| Curriculum & Question Bank | ✅ ~90% (Phase 16, 2026-05-27) | Phase 16a-i closed. **Models**: Subject extended with grade_levels + national_subject_id; new Unit, Topic, NationalSubject, NationalUnit, NationalTopic, Question, QuestionOption, QuestionDraft, HomeworkTemplate, LessonPlanTemplate. Single Alembic migration `2026_05_27_021_curriculum.py` for all 10+ tables. **Endpoints**: school-local CRUD (`/curriculum/{subjects,units,topics}`, `/curriculum/tree`, `/curriculum/adopt-subject`); Ministry-side (`/ministry/national-curriculum/*` gated `school:create`); bulk CSV (`/bulk/curriculum` + `/ministry/bulk/national-curriculum`); cross-index (`/curriculum/topics/{id}/resources` + `/curriculum/coverage`); generic tag (`/curriculum/tag`); question bank (`/questions`, `/question-drafts` + approve/reject); assessment composition + auto-grading (`/assessments/{id}/compose` + `/assessments/{id}/auto-grade` covering MCQ/TF/SA); homework + lesson-plan templates (`/homework-templates` + `/lesson-plan-templates`) with HoD-publish + instantiate-to-class + sync-from-source. **Attachment polish**: MIME allowlist extended with Office (.xlsx/.docx/.pptx/.xls) + text/csv; size cap 10MB → 25MB; 8 new owner_kinds. **UI**: teacher-web `/curriculum`, `/curriculum/[topicId]`, `/question-bank`, `/question-bank/drafts`; admin-web `/curriculum`, `/curriculum/coverage`, `/curriculum/adopt`, `/question-bank/review`; admin-web Ministry `/ministry/curriculum` with publish + new-subject modal. **Sample data**: `scripts/import_zimsec.py` ships 3 placeholder ZIMSEC subjects for dev/test with a "replace before production" notice. **Tests**: +48 backend tests (729 total: 470 academics + 130 communications + 90 finance + 39 identity). **Audit invariants** (ADR 018, ADR 022): no question text, no answers, no template titles, no homework bodies in `target` / `details`; only IDs + counts + admin-supplied rejection_reason. **Open follow-ups**: attachment cloning on instantiate, OCR (Phase 17), thumbnails (Phase 17), curriculum versioning (Phase 17+), cross-school template sharing (Phase 18), essay auto-grading (Phase 18). |
