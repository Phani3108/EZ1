"""
Dropout Risk Engine — Rules-Based Intelligence Layer
======================================================
Pure computation engine: takes attendance + fee data, returns risk assessment.
No I/O — all data fetching is done by the route handlers.
...
Attendance Signals:
  - Consecutive absences: 3→+25, 5→+40, 10→+60  (mutually exclusive, pick highest)
  - 30-day rate: <80%→+15, <65%→+30, <50%→+45   (mutually exclusive, pick worst)
...
class DropoutRiskEngine:
    ...
    def compute(
        ...
        """
        Compute dropout risk for a single student.

        Parameters
        ----------
        attendance_days : list[dict]
            Each item: {"date": "YYYY-MM-DD", "status": "P"|"A"|"L"}
            Should be sorted ascending by date (most recent last).
        invoices : list[dict]
            Each item: {"total_amount": float, "paid_amount": float,
                        "due_date": "YYYY-MM-DD", "status": str}
        today : date, optional
            Override for deterministic testing (default: date.today()).

        Returns
        -------
        dict with keys: risk_score, risk_band, signals
        """
        ...
        # Score = sum of points, capped at 100
    ...
    def _attendance_signals(days: list[dict]) -> list[dict]:
        ...
        # 1. Consecutive absences (from most recent day backward)
        consecutive = 0
        ...
        # 2. Attendance rate over the observation window
        total = len(days)
    ...
    def _fee_signals(invoices: list[dict], today: date) -> list[dict]:
        ...
        for inv in invoices:
            if inv.get("status") in ("PENDING", "PARTIAL", "OVERDUE"):
                total_outstanding += float(inv.get("total_amount", 0)) - float(inv.get("paid_amount", 0))
