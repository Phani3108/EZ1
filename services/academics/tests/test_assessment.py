...
os.environ["DATABASE_URL"] = "sqlite:///./test_academics_assessment.db"
...
engine = create_engine("sqlite:///./test_academics_assessment.db", connect_args={"check_same_thread": False})
...
def setup_db():
    ...
    if os.path.exists("./test_academics_assessment.db"):
        try:
            os.remove("./test_academics_assessment.db")
        except OSError:
            pass
...
# PH3 / BUG-007: academics no longer decodes JWTs. Tests inject the
# gateway-provided headers directly.
def _gateway_headers(user_id=None, school_id=None, roles=None):
    return {
        "X-Gateway-Token": os.environ.get(
            "INTERNAL_SERVICE_TOKEN", "test-internal-token-DO-NOT-USE-IN-PRODUCTION",
        ),
        "X-User-Id": str(user_id or TEACHER_1),
        "X-School-Id": str(school_id or SCHOOL_A),
        "X-User-Roles": ",".join(roles or ["Teacher"]),
    }


def _teacher_headers(user_id=None, school_id=None):
    return _gateway_headers(user_id, school_id, ["Teacher"])


def _admin_headers(user_id=None, school_id=None):
    return _gateway_headers(user_id, school_id, ["Admin"])


def _parent_headers(user_id=None, school_id=None):
    return _gateway_headers(user_id, school_id, ["Parent"])
...
class TestAPIEndpoints:
    @patch("app.api.assessment_routes.verify_teacher_class_authorization", new_callable=AsyncMock)
    ...
    @patch("app.api.assessment_routes.verify_parent_student_authorization", new_callable=AsyncMock)
...
class TestGradebook:
    """T-015: cross-assessment gradebook for a class/term.

    The endpoint returns a sparse matrix (students × assessments) with
    each student's marks for every assessment in the filter window.
    """

    def _seed(self, db):
        """Two assessments, two students, four marks."""
        svc = _svc(db)
        a1 = svc.create_assessment(
            SCHOOL_A, YEAR_A, TERM_A, CLASS_A, SUBJECT_MATH,
            "Quiz 1", "QUIZ", date(2026, 3, 1), Decimal("20"), TEACHER_1,
        )
        a2 = svc.create_assessment(
            SCHOOL_A, YEAR_A, TERM_A, CLASS_A, SUBJECT_MATH,
            "Test 1", "TEST", date(2026, 3, 15), Decimal("50"), TEACHER_1,
        )
        svc.bulk_upsert_marks(
            SCHOOL_A, uuid.UUID(a1["id"]),
            [{"student_id": str(STUDENT_1), "marks": "18"},
             {"student_id": str(STUDENT_2), "marks": "14"}],
            TEACHER_1,
        )
        svc.bulk_upsert_marks(
            SCHOOL_A, uuid.UUID(a2["id"]),
            [{"student_id": str(STUDENT_1), "marks": "40"},
             {"student_id": str(STUDENT_2), "is_absent": True}],
            TEACHER_1,
        )
        return a1, a2

    def test_empty_class_returns_empty_grid(self, db):
        svc = _svc(db)
        result = svc.class_gradebook(SCHOOL_A, CLASS_A, TERM_A)
        assert result["assessments"] == []
        assert result["rows"] == []

    def test_grid_returns_assessments_chronologically(self, db):
        a1, a2 = self._seed(db)
        result = _svc(db).class_gradebook(SCHOOL_A, CLASS_A, TERM_A)
        ids = [a["id"] for a in result["assessments"]]
        # Quiz 1 (Mar 1) precedes Test 1 (Mar 15)
        assert ids == [a1["id"], a2["id"]]

    def test_grid_rows_per_student_with_correct_marks(self, db):
        a1, a2 = self._seed(db)
        result = _svc(db).class_gradebook(SCHOOL_A, CLASS_A, TERM_A)
        rows = {r["student_id"]: r for r in result["rows"]}
        assert str(STUDENT_1) in rows
        assert rows[str(STUDENT_1)]["marks"][a1["id"]]["marks"] == 18.0
        assert rows[str(STUDENT_1)]["marks"][a2["id"]]["marks"] == 40.0
        assert rows[str(STUDENT_1)]["graded_count"] == 2

    def test_is_absent_excluded_from_average(self, db):
        a1, a2 = self._seed(db)
        result = _svc(db).class_gradebook(SCHOOL_A, CLASS_A, TERM_A)
        rows = {r["student_id"]: r for r in result["rows"]}
        s2 = rows[str(STUDENT_2)]
        # Student 2 was absent on Test 1 → only Quiz 1 counts.
        assert s2["graded_count"] == 1
        # 14/20 = 70%
        assert s2["average_pct"] == 70.0

    def test_subject_filter_narrows_assessments(self, db):
        a1, a2 = self._seed(db)
        # Add an English assessment that should be excluded by the filter
        svc = _svc(db)
        svc.create_assessment(
            SCHOOL_A, YEAR_A, TERM_A, CLASS_A, SUBJECT_ENG,
            "Eng Quiz", "QUIZ", date(2026, 3, 5), Decimal("10"), TEACHER_1,
        )
        result = svc.class_gradebook(
            SCHOOL_A, CLASS_A, TERM_A, subject_id=SUBJECT_MATH,
        )
        ids = {a["id"] for a in result["assessments"]}
        assert ids == {a1["id"], a2["id"]}

    def test_other_school_returns_empty(self, db):
        self._seed(db)
        result = _svc(db).class_gradebook(SCHOOL_B, CLASS_A, TERM_A)
        # Cross-tenant isolation: school B sees no rows or assessments
        # for class A's data.
        assert result["assessments"] == []
        assert result["rows"] == []
