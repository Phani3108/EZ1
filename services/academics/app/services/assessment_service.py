...
class AssessmentService:
    ...
    def class_gradebook(
        ...
        """Return the term-wide gradebook for a class: every assessment
        on the column axis, every student on the row axis, with each
        cell carrying the student's mark for that assessment (or None).
        ...
        Shape::

            {
              ...
              "term_id": str | None,
              "subject_id": str | None,
              "assessments": [{id, name, type, date, subject_id, max_marks}, ...],
              "rows": [
                {
                  "student_id": str,
                  "marks": {
        ...
        The student list is NOT joined here — the teacher-web client
        already has the roster from `student.getEnrollmentsByClass`. We
        return students-with-marks; students with zero marks are
        included implicitly by the client merging the roster with this
        sparse view.

        Privacy: marks are sensitive. The endpoint is teacher-only at
        the gateway. Audit-logging the read is intentionally NOT done
        here because gradebook is a hot view — the dropdown-driven
        filter would create excessive audit volume. The
        gradebook-export path (Phase 11g) IS audited.
        """
        # Step 1: assessments matching the filters, oldest-first by
        # date so the gradebook columns read left-to-right
        # chronologically.
        aq = self.db.query(Assessment).filter(
            Assessment.class_id == str(class_id),
            Assessment.school_id == str(school_id),
            Assessment.deleted_at.is_(None),
        )
        if term_id:
            aq = aq.filter(Assessment.term_id == str(term_id))
        if subject_id:
            aq = aq.filter(Assessment.subject_id == str(subject_id))
        assessments = aq.order_by(Assessment.date.asc(), Assessment.name.asc()).all()
        ...
        # Step 2: marks for those assessments in one query.
        marks = (
            self.db.query(Mark)
            .filter(
        ...
        # Also build assessment_id → max_marks lookup for average calc.
        max_for = {a.id: float(a.max_marks) for a in assessments}
        ...
        for m in marks:
            ...
            row["marks"][m.assessment_id] = cell
            if cell["marks"] is not None and not cell["is_absent"]:
                row["_total"] += cell["marks"]
                row["_max"] += max_for.get(m.assessment_id, 0.0)
                row["_graded"] += 1

        rows = []
        for sid, data in rows_by_student.items():
            avg = (
                round(data["_total"] / data["_max"] * 100, 1)
                if data["_max"] > 0
                else None
            )
            rows.append({
                ...
                "marks": data["marks"],
                "graded_count": data["_graded"],
                "average_pct": avg,
            })
        # Stable sort by student_id so the table doesn't shuffle
        # between refreshes.
        ...
        return {
            ...
            "term_id": str(term_id) if term_id else None,
            "subject_id": str(subject_id) if subject_id else None,
            "assessments": [self._ser_assessment(a) for a in assessments],
            "rows": rows,
        }
