...
class SchoolService:
    ...
    def _ser_school(self, s: School) -> dict:
        return {"id": str(s.id), "name": s.name, "country": s.country,
                ...
                # PH8-1 / DEC-009 tenancy tier surface. `getattr` defaults
                # are for back-compat with tests that mock School rows
                # without the new columns.
                "tenancy_tier": getattr(s, "tenancy_tier", "shared"),
                "district_routing_code": getattr(s, "district_routing_code", None),
