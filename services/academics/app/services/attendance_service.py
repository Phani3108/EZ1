...
from app.models.attendance import (
    AttendanceRecord, SyncBatch, ProcessedClientEvent, PERIOD_DAILY,
)
...
class AttendanceService:
    ...
    def process_sync_batch(
        ...
        # Step 1: Collect all (student_id, date, period_number) and
        # client_event_ids from batch. T-002: the period_number is now
        # part of the upsert key — two events for the same student on
        # the same date but different periods are separate rows.
        student_date_period_keys = set()
        client_event_ids = set()
        for e in events:
            period = int(e.get("period_number") or PERIOD_DAILY)
            student_date_period_keys.add(
                (uuid.UUID(str(e["student_id"])), e["date"], period),
            )
        ...
        existing_records = {}
        if student_date_period_keys:
            dates = {d for _, d, _ in student_date_period_keys}
            ...
            for r in rows:
                key = (r.student_id, r.date, r.period_number)
                if key in student_date_period_keys:
        ...
        for e in events:
            ...
            evt_status = e["status"]
            evt_period = int(e.get("period_number") or PERIOD_DAILY)
            ...
            key = (student_id, evt_date, evt_period)
    ...
    def daily_records(self, school_id: uuid.UUID, target_date: date,
                      class_id: uuid.UUID,
                      period_number: Optional[int] = None) -> list[dict]:
        """Return individual attendance records for a class on a given date.

        T-002: when `period_number` is None, returns ALL rows for the
        date (every period). When given a value (including 0), filters
        to that specific period — primary-mode UIs pass 0 to get just
        the daily/homeroom row; secondary-mode UIs pass 1..N for the
        period they're currently marking.
        """
        ...
        if period_number is not None:
            q = q.filter(AttendanceRecord.period_number == period_number)
        records = q.order_by(AttendanceRecord.student_id).all()
