...
class AudienceResolver(Protocol):
    ...
    RULE-4 (Phase 11b) added the TEACHER_CLASSES audience type which
    requires the actor's user id (the teacher creating the announcement)
    to look up which classes they teach. Callers pass `actor_user_id`
    via a keyword arg so older audience types keep working unchanged.
    """
    def resolve(self, school_id: uuid.UUID, audience_type: str,
                class_id: uuid.UUID = None, role: str = None,
                actor_user_id: uuid.UUID = None) -> list[uuid.UUID]: ...
...
class MockAudienceResolver:
    ...
    def add_teacher_class_parents(
        self, school_id: uuid.UUID, teacher_user_id: uuid.UUID,
        users: list[uuid.UUID],
    ):
        """RULE-4: register the "all parents of teacher T's classes"
        set so the resolver can return it for TEACHER_CLASSES."""
        self._users[f"{school_id}:teacher_classes:{teacher_user_id}"] = users
    ...
    def resolve(self, school_id: uuid.UUID, audience_type: str,
        ...
        elif audience_type == "TEACHER_CLASSES" and actor_user_id:
            return self._users.get(
                f"{school_id}:teacher_classes:{actor_user_id}", []
            )
        return []
...
class CommunicationService:
    ...
    def create_announcement(self, school_id: uuid.UUID, title: str, body: str,
                             ...
                             role: str = None,
                             actor_user_id: uuid.UUID = None) -> dict:
        ...
        # RULE-4: TEACHER_CLASSES requires the caller's identity so the
        # resolver knows which classes to look up. Default `actor_user_id`
        # to `created_by` if the caller didn't pass it explicitly — they
        # are the same person in every current call path.
        if audience_type == "TEACHER_CLASSES":
        ...
        user_ids = self.audience_resolver.resolve(
            school_id, audience_type, class_id, role,
    ...
    def get_feed_for_student(self, school_id: uuid.UUID,
        ...
        audience_conditions = [
            ...
            # RULE-4: TEACHER_CLASSES announcements are visible to
            # parents whose child is in any of the teacher's classes.
            # The full filter for "is this parent's child in one of
            # the teacher's classes" would join the academics service;
            # here we surface all TEACHER_CLASSES rows and let the
            # parent's class-scope (passed via `class_id`) narrow them
            # further at the next iteration.
            Announcement.audience_type == "TEACHER_CLASSES",
        ]
        if class_id:
