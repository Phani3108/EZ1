...
async def paynow_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    * Persist gateway breadcrumbs on the transaction.
    ...
    if new_status == "PAID":
        ...
        # BUG-002 fix: amount parsing must not silently fall back. If the
        # webhook reports an unparseable amount we still need to record
        # something (we know the payment was made), but we log + capture
        # the parsing failure in ``last_error`` so an auditor can see it.
        raw_amount = values.get("amount", None)
        if raw_amount is None:
            ...
            try:
                ...
                logger.warning(
                    "Paynow webhook reported unparseable amount %r for ref=%s — "
                    "falling back to txn.amount; err=%s",
                    raw_amount, reference, e,
                )
                paid_amount = Decimal(str(txn.amount))
                txn.last_error = f"unparseable_paynow_amount:{raw_amount!r}"[:500]
        ...
        if "error" in result:
            ...
            payment_id_raw = payment.get("id")
            # BUG-002 fix: cannot silently swallow a missing/malformed
            # payment_id. The payment *was* recorded — we just can't link
            # back to it from the transaction row, which breaks audit /
            # reconciliation. Log + capture in last_error so the row tells
            # the truth, but don't fail the webhook (Paynow expects "Ok").
            if payment_id_raw is None:
                logger.error(
                    "Paynow PAID for ref=%s recorded a payment with no id; "
                    "transaction cannot be linked to payment row",
                    reference,
                )
                txn.last_error = "payment_recorded_without_id"[:500]
            else:
                try:
                    txn.payment_id = uuid.UUID(str(payment_id_raw))
                except (ValueError, TypeError) as e:
                    logger.error(
                        "Paynow PAID for ref=%s could not link payment_id=%r: %s",
                        reference, payment_id_raw, e,
                    )
                    txn.last_error = f"unparseable_payment_id:{payment_id_raw!r}"[:500]
