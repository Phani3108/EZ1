...
class FeesService:
    ...
    def record_payment(self, school_id: uuid.UUID, invoice_id: uuid.UUID,
        ...
        # Postgres uses FOR UPDATE (real row-level lock). SQLite silently
        # ignores it. BUG-010 / Q-007: belt-and-braces — even when the
        # lock is a no-op, we use an optimistic compare-and-swap UPDATE
        # below so a parallel writer can't cause double-spend. Both
        # layers together give:
        #   * Postgres: FOR UPDATE serializes contenders (cheaper than CAS retry).
        #   * SQLite / any engine where FOR UPDATE is missing: CAS catches the
        #     race, the loser sees rowcount=0 and retries.
        try:
        ...
        # Snapshot the value we'll CAS against.
        observed_paid = invoice.paid_amount
        observed_total = invoice.total_amount

        # Check overpayment (against the observed value).
        remaining = observed_total - observed_paid
        if amount > remaining:
            return {"error": "OVERPAYMENT",
                    "message": f"Payment {amount} exceeds remaining balance {remaining}"}

        new_paid = observed_paid + amount
        new_status = (
            "PAID" if new_paid >= observed_total
            else ("PARTIAL" if new_paid > 0 else invoice.status)
        )

        # Insert Payment row first; if the CAS below loses, we'll rollback.
        payment = Payment(
        ...
        # Optimistic CAS via SQLAlchemy's Query.update — works on both
        # Postgres and SQLite. The WHERE clause's `paid_amount == observed_paid`
        # is the CAS predicate; if rowcount != 1 someone else committed
        # a concurrent change between our load and now.
        # `synchronize_session=False` tells the ORM not to try to
        # propagate the change to in-memory instances — we'll expire the
        # invoice instance ourselves below so the next refresh sees fresh
        # DB state.
        rows = self.db.query(Invoice).filter(
            Invoice.id == invoice_id,
            Invoice.school_id == school_id,
            Invoice.paid_amount == observed_paid,
        ).update(
            {Invoice.paid_amount: new_paid, Invoice.status: new_status},
            synchronize_session=False,
        )
        if rows != 1:
            # CAS lost. Roll back the pending Payment insert.
            self.db.rollback()
            return {
                "error": "CONCURRENT_WRITE_LOST",
                "message": "Another payment landed on this invoice concurrently; "
                           "retry the request.",
            }

        # Discard the stale ORM-tracked invoice so commit doesn't replay
        # its old `paid_amount` value back into the DB.
